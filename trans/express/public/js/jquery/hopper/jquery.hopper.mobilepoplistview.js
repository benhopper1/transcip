...
var MobilePopListView = function(inJrefOfThis, inJsonStruct){
	var _this = this;
	var theDivRef;
	var itemHash = {};
	var closeTimer;
	...
	this.open = function(){
		...
		if(options.containerCss){
			$(theDivRef).parent().css(options.containerCss);
		}
...
$.fn.MobilePopListView = function(inAction, inJsonStruct){
	var mobilePopListView = $(this).data("MobilePopListViewInstance");
	if(!(mobilePopListView) || inAction == 'create'){
		inJsonStruct['divRef'] = $(this);
		console.log('mobilePopListView CREATED');
		var mobilePopListView = new MobilePopListView($(this), inJsonStruct);
		$(this).data("MobilePopListViewInstance", mobilePopListView);
		return ;
	}

	if(inAction == 'test'){
		console.log('testing123');
		mobilePopListView.test();
	}

	if(inAction == 'open'){
		console.log('open');
		mobilePopListView.open();
	}

	if(inAction == 'addItem'){
		console.log('addItem');
		return mobilePopListView.addItem(inJsonStruct);
	}

	if(inAction == 'deleteItem'){
		console.log('deleteItem');
		return mobilePopListView.deleteItem(inJsonStruct);
	}

	if(inAction == 'clear'){
		console.log('clear');
		return mobilePopListView.clear(inJsonStruct);
	}

	if(inAction == 'setHeading'){
		console.log('setHeading');
		return mobilePopListView.setHeading(inJsonStruct);
	}

	if(inAction == 'close'){
		console.log('close');
		return mobilePopListView.close(inJsonStruct);
	}

	if(inAction == 'chainOpen'){
		console.log('chainOpen');
		return mobilePopListView.chainOpen(inJsonStruct);
	}

	if(inAction == 'append'){
		console.log('append');
		return mobilePopListView.append(inJsonStruct);
	}
