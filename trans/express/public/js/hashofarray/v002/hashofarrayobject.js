console.log("##########################################################");
console.log("-- H a s h   O f  A r r a y   O b j e c t _ V002 -- Loading");
console.log("##########################################################");
...
//uses hash for duplicate lookups......

var HashOfArrayObject_v002 = function(inAllowDuplicates){
	var _this = this;
	var hash = {};
	...
	this.encodeItemEntry = function(inItem){
		var encodedResult = 
			{
	...
	this.existForKey = function(inKey, inEncodedItem){
		...
		for(var theArrayForKeyIndex in theArrayForKey){
			...
			if(theArrayForKey[theArrayForKeyIndex].hashCode == inEncodedItem.hashCode){
				console.log('existForKey: TRUE');
				return true;
			}
		}
	...
	this.add = function(inKey, inValue){        
		...
		if(allowDuplicates){
			hash[inKey].push(_this.encodeItemEntry(inValue));
		}else{


				var encodedItem = _this.encodeItemEntry(inValue);
				if(!(_this.existForKey(inKey, encodedItem))){
					hash[inKey].push(encodedItem);
				}
	...
	this.removeItemFromSpecificHash = function(inKey, inValue){
		inValue = _this.encodeItemEntry(inValue);
		...
		for(var i = 0; i < arrayOfValues.length; i++){
			if(arrayOfValues[i].hashCode == inValue.hashCode){
	...
	this.getEncodedArrayFromHash = function(inKey){
		if(!(inKey)){console.log('key was empty'); return false;};
		inKey = inKey.toString();
		var attemptValue = hash[inKey];
		if(!(attemptValue)){
	...
	this.getArrayFromHash = function(inKey){
		...
		var decodedArray = [];
		for(attemptValueIndex in attemptValue){
			decodedArray.push(_this.decodeItemEntry(attemptValue[attemptValueIndex]));
		}
		return decodedArray;
	}
	...
	this.getItemFromArray = function(inKey, inIndex){
		...
		if(inIndex < arrayOfItems.length){            
			return _this.decodeItemEntry(arrayOfItems[inIndex]);
		}
		return false;  
	...
	this.getFirstKeyByValue = function(inValueToCheck){
		inValueToCheck = _this.encodeItemEntry(inValueToCheck);
		var keys = _this.getKeys();
		for(index in keys){
			var storedArrayForKey = _this.getEncodedArrayFromHash(keys[index]);
			for(arrayIndex in storedArrayForKey){
				if(storedArrayForKey[arrayIndex].hashCode == inValueToCheck.hashCode){
					return keys[index];
				}
			}
		}
