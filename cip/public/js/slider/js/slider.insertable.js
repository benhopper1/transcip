		...
		console.log('CONTACT SLIDER FILE LOADED');


		var ContactsSlider = function(inJstruct){
			var _this = this;
			...
			instantOptions = $.extend(instantOptions, inJstruct);

			var containerId = instantOptions.containerId;

			var slideIndex = 0;

			var slideContentHash = {};
			...
			var _SlideshowTransitions = 
				...
				var parentWidth = jssor_slider2.$Elmt.parentNode.clientWidth;
					if (parentWidth)
						jssor_slider2.$ScaleWidth(Math.min(parentWidth, instantOptions.width));
					else
				...
				//==================================================================================
				//  REDRAW AFTER A SLIDE APPEND OR REMOVE
				//==================================================================================
				this.redraw = function(){
					slideIndex = 0;
					var originalHTML =
						'<center><div id="slider2_container_0" style="position: relative; width: 600px;height: 300px;">' + '' +
							...
							'</div>' + '' +
							'<div id="slidesDiv" u="slides" style="cursor: move; position: absolute; left: 0px; top: 0px; width: ' + slideWidth + '; height: ' + slideHeight + ';overflow: hidden;">' + '' +
							'</div>' + '' +
						'</div></center>'
					;

					$('#slider2_container_0').remove();

					$('#' + containerId).html(originalHTML);

					var index = 0;
					for(var slideContentHashIndex in slideContentHash){
						$('#slidesDiv').append(slideContentHash[slideContentHashIndex]);
						$('.contactClickable').last().attr('slideIndex', index);
						$('.contactClickable').last().attr('id', containerId + '_' + 'slideIndex_' + index++);
					}

					jssor_slider2 = new $JssorSlider$("slider2_container_0", options);

					jssor_slider2.$On($JssorSlider$.$EVT_CLICK, function(slideIndex){
						var theLookupId = _this.getLookupIdByIndex(slideIndex);
						if(instantOptions.onClick){
								instantOptions.onClick(slideIndex, theLookupId);
						}
					...
					jssor_slider2.$On($JssorSlider$.$EVT_SWIPE_END , function(slideIndex){
						if(slideIndex % 1 === 0){
							...
							var theLookupId = _this.getLookupIdByIndex(slideIndex);
							if(instantOptions.onSwipe){
								instantOptions.onSwipe(slideIndex);
							}
				...
				//==================================================================================
				//  ADD Image as SLIDE or oerwrite existing
				//==================================================================================
				this.addImageSlide = function(inLookupId, inImageUrl){
					var html = '<div lookupId="' + inLookupId + '" class="contactClickable " ><img class="' + instantOptions.imageClass + '" style="height:93%;" src="' + inImageUrl + '" ></img></div>';
					_this.addSlide(inLookupId, html);
				}

				//==================================================================================
				//  ADD SLIDE or oerwrite existing
				//==================================================================================
				this.addSlide = function(inLookupId, inContent){
					slideContentHash[inLookupId] = inContent;
				}

				//==================================================================================
				//  REMOVE SLIDE
				//==================================================================================
				this.removeSlide = function(inLookupId){
					delete(slideContentHash[inLookupId]);
				}

				//==================================================================================
				//  CLEAR SLIDE
				//==================================================================================
				this.clear = function(inLookupId){
					slideContentHash = {};
				}
				...
				//  SET SIZE SLIDE
				//==================================================================================
				this.setSlideSize = function(inWidth, inHeight){
					slideWidth = inWidth;
					slideHeight = inHeight;
				}

				this.reportOnClick = function(inLookupId){
					if(!(typeof Android === 'undefined')){
						Android.onActionRequest(JSON.stringify(
							{
								request:'contactSliderSelected',
				...
				//==================================================================================
				//  ADD ARRAY OF IMAGES TO SLIDES
				//==================================================================================
				// @ param: [{lookupId:767676,imageUrl:'myPath'}, {lookupId:767677,imageUrl:'myPath'}]
				this.addImageSlideArray = function(inArray){
					for(var inArrayIndex in inArray){
						_this.addImageSlide(inArray[inArrayIndex].lookupId, inArray[inArrayIndex].imageUrl);
					}
				}

				this.getLookupIdByIndex = function(inIndex){
					var div = $('#' + containerId + '_' + 'slideIndex_' + inIndex);
					return $(div).attr('lookupId');
				}

				this.geIndexBytLookupId = function(inLookupId){
					var theIndex = 0;
					for(var theKey in slideContentHash){
						if(theKey == inLookupId){
							return theIndex;
						}
						theIndex++;
					}
				...
				this.setSlideByIndex = function(inIndex){
					jssor_slider2.$GoTo(inIndex);
				}
				this.setSlideByLookupId = function(inlookupId){
					var theIndex = _this.geIndexBytLookupId(inlookupId);
					jssor_slider2.$GoTo(theIndex);
				}
